...
export default function UserList() {
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			const data = await getAllUsers(BASE_URL, token)
			setAllUsers([...data])
		}
		fetchData()
	}, [BASE_URL])
	...
	return (
		<div className="mb-10">
			<h2 className="pl-5 sm:pl-0 text-2xl font-bold uppercase sm:mt-0 mt-4 mb-4 sm:mb-6">Manage Users</h2>
			<input className="ml-5 sm:ml-0 mb-8 bg-transparent text-lg py-1" type="text" onChange={handleChange} value={filter} />
			<svg className="inline ml-4" width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			{filter ? <span className="cursor-pointer ml-5 hover:text-red-500" onClick={() => setFilter('')}>Clear</span> : null}
			<div className="space-y-4 w-full flex flex-col items-center sm:items-start">
				{allUsers.filter(user => {
					return user.firstName.toUpperCase().includes(filter.toUpperCase()) || user.lastName.toUpperCase().includes(filter.toUpperCase()) || user.username.toUpperCase().includes(filter.toUpperCase())
				}).map(user => {
					return <UserDetail key={uuid()} user={user} />
				})}
			</div>
		</div>
	)
