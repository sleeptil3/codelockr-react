...
export default function View({ filter, setFilter }) {
	...
	return (
		<div className="ml-2">
			{userData.friends.length ? <><h1 className="text-2xl font-bold">{filter ? `${userData.friends.find(friend => friend._id === filter).firstName} 's Shared Snippets` : "All Friends Shared Snippets"}</h1>
				<div className="mb-4 my-4 flex items-center">
					...
					<div>
						<input className="focus:ring-0 bg-transparent text-md px-1 py-1" type="text" onChange={handleChange} value={search} />
					</div>
					{search ? <span className="cursor-pointer ml-3" onClick={handleClear}>Clear</span> : null}
				</div>
			</>
				: <div><h1 className="text-2xl font-bold mt-8 ml-8">Find Friends</h1> <h3 className="text-xl font-light mt-8 ml-8">Get started by creating your first snippet!</h3></div>}
			<div className="grid gap-10 mt-2 pr-8 w-full grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3">
