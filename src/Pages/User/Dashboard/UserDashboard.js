...
export default function UserDashboard() {
	...
	return (
		<div className="flex h-max justify-start items-start w-screen">
			<div className="bg-gray-300 px-8 py-4 space-y-4 flex-col shadow-lg flex-shrink-0">
				...
				<div>
					...
					<ul className="ml-2 space-y-1">
						{userData.folders.sort((a, b) => a.title.toUpperCase() < b.title.toUpperCase() ? -1 : 1).map(folder => {
							return <li className={`my-1 cursor-pointer py-1 px-2 w-max ${filter === folder._id ? 'bg-darkBlue text-gray-50 rounded-md' : ''}`} id={folder._id} onClick={handleFilter} key={uuid()}>{folder.title}</li>
						})}
					</ul>
				</div>
				<div>
					<Link to={`/user/${userData.username}/dashboard/addsnippet`} onClick={() => setFilter("")} className="bg-darkBlue tracking-widest rounded-md shadow-md px-3 py-1 text-sm text-gray-50 font-thin flex justify-center items-center">
