...
export default function UserDashboard() {
	...
	return (
		<div className="flex justify-start items-start w-screen h-screen">
			<div className="relative top-20 left-5 w-max h-max ">
				<div className="bg-gray-900 w-max space-y-4 flex-col rounded-xl px-8 py-4 shadow-lg flex-shrink-0">
					{userData.folders.length ? <Link to={`/user/${userData.username}/dashboard`} className="cursor-pointer text-lg font-normal" onClick={() => setFilter('')}>All Snippets</Link> : null}
				...
				<div className="mt-4 flex justify-center">
					<Link to={`/user/${userData.username}/dashboard/addsnippet`} onClick={() => setFilter("")} className="w-4/5 bg-gradient-to-tr from-darkBlue to-red-800 tracking-widest rounded-md shadow-lg px-2 py-1 text-sm text-gray-50 font-thin flex justify-center items-center">
