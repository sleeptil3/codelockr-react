		...
		<div className="flex flex-col sm:flex-row justify-start items-center sm:items-start w-screen">
			<div className="sm:ml-5 w-full sm:w-max h-max ">
					...
					{userData.folders.length ? <div>
						...
						<ul className="ml-2 text-sm space-y-1">
							{userData.folders.sort((a, b) => a.title.toUpperCase() < b.title.toUpperCase() ? -1 : 1).map(folder => {
								return <li className={`my-1 cursor-pointer py-1 px-2 w-max ${filter === folder._id ? 'hover:text-gray-50 bg-gradient-to-tr from-darkBlue to-red-800 text-gray-50 rounded-md' : 'hover:text-red-600'}`} id={folder._id} onClick={handleFilter} key={uuid()}>{folder.title}</li>
							})}
						</ul>
					</div>
						: null}
				...
				<div className="sm:hidden bg-gray-900 -mt-4 w-full py-2 shadow-lg flex justify-evenly items-center">
					...
					<select value={filter} name="snippetFilter" onChange={handleFilter} className="bg-transparent w-3/4 tracking-widest">
