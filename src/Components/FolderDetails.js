...
export default function FolderDetails({ folder, username }) {
	...
	return (
		<div>
			<div className={`transition-transform transform hover:scale-105 bg-gradient-to-r from-darkBlue to-red-500 flex justify-between min-w-max font-normal rounded-lg shadow-sm px-2 py-1 space-x-4`}>
				...
				<div className="flex justify-start items-center space-x-4">
					<svg className="self-center cursor-pointer shadow-lg" id="delete" onClick={() => setDeleteMode(true)} width="30" height="24" viewBox="0 0 30 24" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M13.2071 18.7071L18.5 13.4142L23.793 18.7071L25.2071 17.2929L19.9142 12L25.2071 6.70706L23.793 5.29294L18.5 10.5858L13.2071 5.29294L11.793 6.70706L17.0858 12L11.793 17.2929L13.2071 18.7071Z" fill="#F9FAFB" />
						<path
			...
			{editMode ? (
				<form className={`sm:ml-5 mt-5 flex pb-4 sm:pb-0 sm:flex-row space-y-4 sm:space-y-0 sm:mb-8 flex-col sm:items-end`} onSubmit={handleSubmit}>
					...
					<input className="focus:ring-0 mr-8 w-full sm:w-auto bg-transparent border-t-0 border-l-0 border-r-0 border-b-2 py-0 px-1 tracking-widest" type="text" autoComplete="off" id="title" value={formData.title} onChange={handleChange} />
					<div className="sm:w-full space-x-6">
						<button className="btn-primary w-1/3 px-2 py-1" type="submit">
							Save
						</button>
						<button className="sm:ml-5 w-1/3 btn-secondary px-2 py-1" onClick={() => setEditMode(false)}>
							Cancel
						</button>
					</div>
				</form>
			...
			{deleteMode ? (
				<div className="sm:ml-5 mt-5 space-y-4 sm:w-max p-5 flex-col items-baseline border-2 rounded-lg shadow border-red-600" onSubmit={handleSubmit}>
					...
					<button className="focus:ring-0 btn-primary px-2 py-1 item-grow-0 mt-4 bg-red-600" id="edit" onClick={handleDelete}>
						Yes, I'm sure!
					</button>
					<button className="focus:ring-0 ml-5 btn-secondary px-2 py-1 item-grow-0" id="delete" onClick={() => setDeleteMode(false)}>
						No, cancel!
					</button>
				</div>
			) : null}
