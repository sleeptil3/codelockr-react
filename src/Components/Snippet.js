...
export default function Snippet({ parentFolder, readOnly, title, code, isPrivate, parseFormat, notes, snippet_id, owner }) {
	...
	if (!userData.username) return <h1 className="text-2xl text-gray-50 font-normal">Loading</h1>
		...
		return (
			<div className="shadow-lg sm:border-4 border-gray-900 py-2 px-4 sm:px-8 space-y-2 pb-4 sm:pt-4 sm:pb-8 sm:space-y-4 bg-gradient-to-br from-darkBlue to-black">
				<div className="flex justify-between items-center">
					<div className="">
						<h2 className="text-lg sm:text-xl text-gray-50 font-thin">{title}</h2>{isPrivate ? <span className="text-xs">Private</span> : null}
					</div>
					{readOnly ? <p className="text-xs text-gray-200">Owner: {owner}</p>
				...
				<pre className="font-mono text-xs">
					<code className={`font-mono language-${parseFormat} rounded-md border border-gray-50 p-3 xl:h-96`}>
						{code}
					</code>
				</pre>
				{ notes ?
