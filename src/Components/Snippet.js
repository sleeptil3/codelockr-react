...
export default function Snippet({ readOnly, title, code, parentFolder, parseFormat, notes, snippet_id, owner }) {
	...
	if (!userData.username) return <h1 className="text-2xl text-gray-50 font-normal">Loading</h1>
		...
		return (
			<div className="shadow-lg border-4 border-gray-900 py-4 pb-7 px-8 space-y-4 bg-gradient-to-br from-darkBlue to-black">
				<div className="flex justify-between items-center">
					<h2 className="text-2xl text-gray-50 font-thin">{title}</h2>
				...
				<pre className="text-sm">
					<code className={`language-${parseFormat} rounded-md border border-gray-50 p-3 xl:h-96`}>
						{code}
					</code>
				</pre>
				{ notes ?
